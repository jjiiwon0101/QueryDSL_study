...
//        teamRepository.save(teamB);
    ...
    void testJPQL() {
        ...
        System.out.println("jpqlQuery = " + jpqlQuery);
        System.out.println("\n\n\n");
    }
...
//        QMember m = QMember.member;
        //when
        Member foundMember = factory.select(member)
                .from(member)
        ...
        System.out.println("\n\n\n");
        System.out.println("foundMember = " + foundMember);
        System.out.println("foundMember.getTeam() = " + foundMember.getTeam().getName());
        System.out.println("\n\n\n");
    }
...
//                        .and(member.age.eq(searchAge))
         ...
         * member.userName.inNotNull() // 이름이 is not null
         * member.age.in(10, 20) // age in (10,20)
         * member.age.notIn(10 ,20) // age not in (O1.,20)
         * member.age.between(10, 30) // age between 10 , 30
         * member.age.goe(10, 30) // age 10 >= 30
         * member.age.gt(30) // age  > 30
         * member.age.loe(30) // age <= 30
         * member.age.lte(30) // age < 30
         * member.userName.like("_김%") // userName LIKE '_김%' (언더바: 위치 지정)
    ...
    void testGroupBy() {
        ...
        List<Long> result = factory.select(member.age.count())
                .from(member)
                .orderBy(member.age.asc()) //정렬 먼저 작성 가능
                .groupBy(member.age)
    ...
    /**
     * ex) 회원과 팀을 조인하면서, 팀 이름이 teamA인 팀만 조회할거고, 회원은 모두 조회
     * SQL: SELECT m.*, t.* FROM tbl_member m LEFT JOIN tbl_member t ON m.team_id = t.id AND t.name = 'teamA
